...
from app.forms.auth_forms import LoginAdminForm
...
# ====================================================================================================================================================
    ...
    def Login_Admin(self):
        ...
        intentos_fallidos = session.get("login_intentos_admin", 0)
        ...
        if request.method == "POST":
            ...
            if not form.validate_on_submit():
                return render_no_cache("auth/login_admin.html", **ctx)
            ...
            username = form.username_admin.data
            ...
            try:
                ...
                if not data_user:
                    ...
                    ctx["mostrar_recaptcha"] = session["login_intentos_admin"] >= INTENTOS_PARA_RECAPTCHA
                ...
                if not self._validar_rol_admin(data_user["FK_ID_Rol"]):
                    # Auditar intento fallido con usuario existente                    
                    Auditoria_Session(data_user["ID_Usuario"], ip, "FAILED_LOGIN", user_agent)
                ...
                # LOGIN EXITOSO
                session.pop("login_intentos_admin", None)
                ...
                if data_user.get("Doble_Factor_Activo") == "ACTIVE":
                    ...
                    session["mfa_rol_esperado"] = "Admin"                    
                    session["mfa_login_url"] = url_for("auth.login_admin")
    ...
    def _validar_rol_admin(self, role_id):
        """Obtiene el ID del rol para comparar con el rol del usuario"""
        try:
            if role_id is None:
                return False
                
            id_rol_requerido = sp_obtener_roles("Admin")
            if id_rol_requerido != role_id:
                return False
            return True
        
        except Exception as e:
            print(f"[ERROR] _validar_rol_admin: {e}")
            return False
...
# ====================================================================================================================================================
    ...
    def Logout(self):
        ...
        try:
            ...
            response = make_response(redirect(url_for("auth.login_admin"), 303))
            unset_jwt_cookies(response)
            session.clear()
            return response
        
        except Exception as e:
            print(f"[ERROR] Logout admin: {e}")
            response = make_response(redirect(url_for("auth.login_admin"), 303))
            unset_jwt_cookies(response)
            session.clear()
            return response